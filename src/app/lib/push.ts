...
/**
 * Send FCM push notification for an SOS alert.
 * Queries all device tokens for the target user and sends a high-priority SOS notification.
 * Never includes Twilio credentials or raw provider response data in the payload.
 *
 * Returns { sent, failed } counts. If Firebase is not initialized, logs a warning
 * and returns { sent: 0, failed: 0 } — does not throw.
 */
export async function sendSosPushNotification(
  ...
  try {
    if (getApps().length === 0) {
      console.warn('[push] Firebase not initialized — skipping SOS push notification for alert', payload.alertId)
      return { sent: 0, failed: 0 }
    }
    messaging = getMessaging(getApp())
  } catch {
    console.warn('[push] Firebase initialization error — skipping SOS push notification for alert', payload.alertId)
    return { sent: 0, failed: 0 }
  }
  ...
  // Only include safe, non-sensitive fields — never Twilio credentials or provider metadata
  const message = {
    notification: {
      title: 'SOS Alert',
      body: `${payload.fromUserName} needs emergency help!`,
    },
    data: {
      type: 'SOS',
  ...
  try {
    ...
    if (unregisteredTokens.length > 0) {
      ...
      console.log(`[push] Removed ${unregisteredTokens.length} unregistered token(s) for user ${userId}`)
    }
    ...
    console.error('[push] FCM SOS send error:', error)
    return { sent: 0, failed: 0 }
  }
}
